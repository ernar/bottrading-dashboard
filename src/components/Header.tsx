...
function BotToggleButton({ running }: { running: boolean }) {
  ...
  const toggle = async () => {
    setBusy(true)
    try {
      await fetch(`${API_URL}/api/bot/${running ? 'stop' : 'start'}`, { method: 'POST' })
    ...
    <header className="bg-gray-900 text-white p-4 border-b border-gray-700">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-3">
          <h1 className="text-2xl font-bold">{platform} Trading Bot</h1>
          {state.last_update && (
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-4 text-sm">
        <div className="bg-gray-800 p-3 rounded">
          ...
          <div className="text-xl font-semibold">${account.balance.toFixed(2)}</div>
        </div>
        <div className="bg-gray-800 p-3 rounded">
          <div className="text-gray-400">Equity</div>
          <div className="text-xl font-semibold">${account.equity.toFixed(2)}</div>
        </div>
        <div className="bg-gray-800 p-3 rounded">
        ...
        <div className="bg-gray-800 p-3 rounded">
          ...
          <div className="text-xl font-semibold">${account.free_margin.toFixed(2)}</div>
        </div>
        <div className="bg-gray-800 p-3 rounded">
          <div className="text-gray-400">Posiciones</div>
          <div className="text-xl font-semibold">{openPositions}</div>
        </div>
        <div className="bg-gray-800 p-3 rounded">
          <div className="text-gray-400">Leverage</div>
          <div className="text-xl font-semibold">{account.leverage}:1</div>
        </div>
      </div>
    </header>
  )
